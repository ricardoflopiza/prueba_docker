          ...
          div(class="conten-ine", 
              ...
              fluidPage(
                ...
                <div class="col-md-12">
                    ...
                    <p class="text-ine">
Esta aplicación permite implementar el estándar de calidad para las estimaciones en encuestas de hogares del INE. A través de ella, los usuarios pueden conocer cuál es la calidad/precisión que tienen las estimaciones generadas a partir de encuestas producidas por el INE u otras encuestas que utilicen muestreo probabilístico estratificado y en 2 etapas. Con esto, se busca poner a disposición de la comunidad una herramienta interactiva para implementar el estándar de calidad del INE, promoviendo el uso adecuado de la información publicada. Esta aplicación permite evaluar la calidad de la estimación de medias, totales, medianas y proporciones.
                    </p>
                </div>
            </div>')
                )
...
server <- function(input, output, session) {
  ...
  output$tabulado  <- renderText({
    calidad::tabla_html(tabuladoOK()) # %>% rename(es = se, cv = coef_var, media = contains("mean"))
  })
